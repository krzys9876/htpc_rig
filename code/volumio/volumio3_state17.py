...
class VolumioWebRadios(VolumioMenu):
    ...
    def __init__(self):
        VolumioMenu.__init__(self,'WEB RADIO:'+chr(0x10)+'sel '+chr(0x11)+'back')
        self.clear()
...
##                    '{: <{:d}}'.format('',MAX)]
    ...
    def setAlbumArt(self,p_albumart):
    ...
    def printGraphics(self,img):
        ...
        if(GUI):
            try:
                ...
                printTimestamp()
                print("[2:{0}]Image display error({1}): {2}".format(errflag,e.errno, e.strerror))
        else:                
...
#            return
    ...
    def setTitle(self,p_artist,p_album,p_title,p_print=False):
    ...
    def readVolumioState(self):
        try:
            ...
            self.setTitle(art,alb,tit,char_print)
            self.setAlbumArt(getVar(js,'albumart'))
    ...
    def shutdown(self):
        ...
        #self.menuState=VolumioReader.IDLE # testowo, bez znaczenia po wykonaniu shutdown
    ...
    def request_file(self,url):
        ...
        if os.path.exists(download_file):
            with open(download_file,'rb') as d_file:
                resp = d_file.read()
            self.wait_for_file(download_file)
            respStatus=os.path.exists(download_file)
